...
const DEFAULT_BUDGETS     = { housing:1500, food:500, transport:300, health:200, entertainment:200, savings:400, other:150 };
const DEFAULT_TRANSACTIONS = [
  { id:1, desc:"Rent",       amount:1400, category:"housing",       month: new Date().getMonth() },
  { id:2, desc:"Groceries",  amount:180,  category:"food",          month: new Date().getMonth() },
  { id:3, desc:"Netflix",    amount:15,   category:"entertainment", month: new Date().getMonth() },
  { id:4, desc:"Gas",        amount:60,   category:"transport",     month: new Date().getMonth() },
  { id:5, desc:"Gym",        amount:45,   category:"health",        month: new Date().getMonth() },
];
...
function guessCategory(chaseCat, desc, categories) {
  const low = (chaseCat||"").toLowerCase(), d = (desc||"").toLowerCase();
  const ids = categories.map(c=>c.id), has = id => ids.includes(id);
  for (const [k,v] of Object.entries(CHASE_CAT_MAP)) if (low.includes(k) && has(v)) return v;
  if ((d.includes("rent")||d.includes("mortgage")) && has("housing")) return "housing";
  if ((d.includes("grocer")||d.includes("whole foods")||d.includes("trader joe")) && has("food")) return "food";
  if ((d.includes("uber")||d.includes("lyft")||d.includes("gas")||d.includes("shell")) && has("transport")) return "transport";
  if ((d.includes("netflix")||d.includes("spotify")||d.includes("hulu")) && has("entertainment")) return "entertainment";
  if ((d.includes("gym")||d.includes("pharmacy")||d.includes("cvs")) && has("health")) return "health";
...
function parseChaseCSV(text, categories) {
  ...
  return lines.slice(1).flatMap((line, i) => {
    ...
    const txDate = clean(cols[0]), description = clean(cols[2]), chaseCat = clean(cols[3]);
    const amountRaw = parseFloat(clean(cols[5]));
    if (isNaN(amountRaw) || amountRaw >= 0) return [];
    const date = new Date(txDate);
    if (isNaN(date.getTime())) return [];
    return [{ id: Date.now()+i, desc:description, amount:Math.abs(amountRaw),
              category:guessCategory(chaseCat,description,categories),
              month:date.getMonth(), date:txDate, source:"chase" }];
  });
}
...
export default function BudgetTool() {
  ...
  // State — all initialised from localStorage with fallback to defaults
  const [activeMonth,   setActiveMonth]   = useState(today.getMonth());
  const [income,        setIncome]        = useState(() => load("bf_income",        4000));
  const [categories,    setCategories]    = useState(() => load("bf_categories",    DEFAULT_CATEGORIES));
  const [budgets,       setBudgets]       = useState(() => load("bf_budgets",       DEFAULT_BUDGETS));
  const [transactions,  setTransactions]  = useState(() => load("bf_transactions",  DEFAULT_TRANSACTIONS));

  const [newTx,         setNewTx]         = useState({ desc:"", amount:"", category: load("bf_categories", DEFAULT_CATEGORIES)[0]?.id || "food" });
  ...
  useEffect(() => save("bf_income",       income),       [income]);
  useEffect(() => save("bf_categories",   categories),   [categories]);
  useEffect(() => save("bf_budgets",      budgets),      [budgets]);
  useEffect(() => save("bf_transactions", transactions), [transactions]);
  ...
  const handleFileChange = e => {
    ...
    reader.onload = ev => {
      ...
      if (!rows.length) { setImportMsg({ type:"error", text:"No transactions found. Make sure it's a Chase CSV export." }); return; }
  ...
  const confirmImport = () => {
    ...
    setImportState(null); setView("transactions");
  };
  ...
  return (
    <div style={{ fontFamily:"'DM Sans','Helvetica Neue',sans-serif", background:"#0f0f13", minHeight:"100vh", color:"#e8e8f0" }}>
        ...
        <div style={{ position:"fixed", inset:0, background:"#000000cc", zIndex:100, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
          <div style={{ background:"#1a1a2e", borderRadius:16, padding:24, width:"100%", maxWidth:320, border:"1px solid #2a2a3e" }}>
            ...
            <div style={{ fontSize:13, color:"#9ca3af", marginBottom:20 }}>
              Transactions in <strong style={{color:"#e8e8f0"}}>{cat(confirmDelete).icon} {cat(confirmDelete).label}</strong> will move to <strong style={{color:"#e8e8f0"}}>{categories.find(c=>c.id!==confirmDelete)?.label}</strong>.
            </div>
            <div style={{ display:"flex", gap:9 }}>
              <button onClick={() => deleteCat(confirmDelete)} style={{ flex:1, padding:"11px", background:"#ef4444", border:"none", borderRadius:9, color:"#fff", fontWeight:700, fontSize:14, cursor:"pointer" }}>Delete</button>
              <button onClick={() => setConfirmDelete(null)} style={{ padding:"11px 16px", background:"#0f0f13", border:"1px solid #2a2a3e", borderRadius:9, color:"#9ca3af", fontWeight:600, fontSize:14, cursor:"pointer" }}>Cancel</button>
            </div>
          </div>
        </div>
      )}
            ...
            {categories.map(c => (
              <div key={c.id} style={{ display:"flex", alignItems:"center", gap:12, padding:"13px 14px", background:"#1a1a2e", borderRadius:12, border:"1px solid #2a2a3e" }}>
                ...
                <div style={{ flex:1 }}>
                  ...
                  <div style={{ fontSize:11, color:"#6b7280", marginTop:1 }}>{transactions.filter(t=>t.category===c.id).length} transactions · Budget {fmt(budgets[c.id]||0)}</div>
                </div>
                <div style={{ display:"flex", gap:6 }}>
                  ...
                  <button onClick={() => setConfirmDelete(c.id)} disabled={categories.length<=1} style={{ padding:"6px 12px", background:"#1e1e2e", border:"1px solid #2a2a3e", borderRadius:7, color:categories.length<=1?"#374151":"#ef4444", fontSize:12, fontWeight:600, cursor:categories.length<=1?"not-allowed":"pointer" }}>🗑️ Delete</button>
                </div>
              </div>
            ))}
            <div style={{ fontSize:12, color:"#4b5563", textAlign:"center", paddingTop:4 }}>
              {categories.length} categories · All data saved automatically to this device
            </div>
          </div>
        )}
      ...
      <div style={{ textAlign:"center", padding:"14px 18px 26px", fontSize:11, color:"#374151" }}>
        💾 Data saved automatically · 📱 Safari → Share → Add to Home Screen
      </div>
    </div>
  );
}
